...
class StockManager
{
  ...
  public function updatePacksQuantityContainingProduct($product, $id_product_attribute, $stock_available, $id_shop = null)
  {
    ...
    foreach ($packs as $pack) {
      ...
      if (
        !((int) $pack->pack_stock_type == Pack::STOCK_TYPE_PACK_BOTH)
  ...
  {
    file_put_contents('/tmp/debug.txt', "[SaveMovement] : Product ID: $productId - Delta: $deltaQuantity\n", FILE_APPEND);
    ...
    if (!Validate::isLoadedObject($product)) {
      file_put_contents('/tmp/debug.txt', "[Error] : Product not found\n", FILE_APPEND);
      return false;
    }

    // 1. Initialisation de l'objet StockMvt (PrestaShop Entity)
    $stockMvt = new StockMvtCore();

    $stockManager = ServiceLocator::get('\\PrestaShop\\PrestaShop\\Adapter\\StockManager');
    $stockAvailable = $stockManager->getStockAvailableByProduct($product, $productAttributeId, $params['id_shop'] ?? null);

    // 2. Mapping des données (Logique identique à votre prepareMovement)
    if (!empty($params['id_order'])) {
      $stockMvt->id_order = (int)$params['id_order'];
    }

    if (!empty($params['id_stock_mvt_reason'])) {
      $stockMvt->id_stock_mvt_reason = (int)$params['id_stock_mvt_reason'];
    }

    $stockMvt->id_stock = $stockAvailable ? (int) $stockAvailable->id : null; // Vous devez fournir cet ID dans $params
    $stockMvt->sign = ($deltaQuantity > 0 ? 1 : -1);
    $stockMvt->physical_quantity = (int)abs($deltaQuantity);
    $stockMvt->date_add = $params['date_add'] ?? date('Y-m-d H:i:s');
    $stockMvt->price_te = $params['price_te'] ?? 0;

    // 3. Gestion de l'employé via le Context
    $context = Context::getContext();
    if (isset($context->employee) && $context->employee->id) {
      $stockMvt->id_employee = (int)$context->employee->id;
      $stockMvt->employee_firstname = $context->employee->firstname;
      $stockMvt->employee_lastname = $context->employee->lastname;
    } else {
      // Optionnel : Forcer un ID employé par défaut (ex: 1) si lancé hors ligne
      $stockMvt->id_employee = 1;
    }

    // 4. Sauvegarde directe en base de données
    try {
      $result = $stockMvt->add();
      file_put_contents('/tmp/debug.txt', "[Debug] Legacy Save Result: " . ($result ? 'Success' : 'Fail') . "\n", FILE_APPEND);
      return $result;
    } catch (\Exception $e) {
      file_put_contents('/tmp/debug.txt', "[Exception] : " . $e->getMessage() . "\n", FILE_APPEND);
      return false;
    }
  }
  ...
  private function prepareMovement($productId, $productAttributeId, $deltaQuantity, $params = [])
  {
    ...
    if ($product->id) {
      ...
      if ($stockAvailable->id) {
        ...
        $stockMvt->setDateAdd($params['date_add'] ?? (new DateTime())->format('Y-m-d H:i:s'));
