...
import { Loader2, AlertTriangle, CheckCircle2, XCircle, Zap, Package, ShoppingCart, Users, FolderTree, Percent, Layers, Ruler } from "lucide-react";
import { fetchProducts, fetchOrders, fetchCustomers, fetchCategories, fetchTaxes, fetchTaxRuleGroups, fetchTaxRules, resetProducts, resetOrders, resetCustomers, resetCategories, resetTaxes, resetTaxRuleGroups, resetTaxRules } from "../services";
...
export function FastResetModalComponent({ open, setOpen, onResetComplete }: FastResetModalProps) {
  ...
  const [steps, setSteps] = useState<ResetStep[]>(
    STEPS.map(s => ({ ...s, status: 'pending', total: 0, deleted: 0, failed: 0 }))
  );
  ...
  const handleFastReset = async () => {
    ...
    try {
      ...
      updateStep(0, { total: products.length });
      ...
      // Step 2: Categories (except id 1 and 2)
      setCurrentStepIndex(1);
      ...
      const categoryIds = categories
        ...
        .map(c => c.id);
      updateStep(1, { total: categoryIds.length });
      ...
      updateStep(3, { total: taxRuleGroups.length });
      ...
      updateStep(4, { total: taxRules.length });
      ...
      if (totalFailed === 0) {
        ...
        toast.warning(`Fast reset complete with ${totalFailed} failures`);
      }
  ...
  const handleClose = () => {
    if (!isRunning) {
      setOpen(false);
      if (completed) {
        setSteps(STEPS.map(s => ({ ...s, status: 'pending', total: 0, deleted: 0, failed: 0 })));
        setCompleted(false);
        setCurrentStepIndex(-1);
      }
    }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            This will delete all data in the following order.
          </DialogDescription>
        </DialogHeader>
        ...
        {!isRunning && !completed && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              This will permanently delete <strong>all</strong> products, categories, taxes, tax rules, customers, and orders. This action cannot be undone.
            </AlertDescription>
          </Alert>
        )}
