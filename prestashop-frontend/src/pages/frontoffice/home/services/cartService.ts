...
export async function cartToCartItems(cart: UserCart): Promise<CartItem[]> {
  ...
  for (const row of rowsArray) {
    ...
    try {
      ...
      }>(`/products/${productId}?display=full&price[price_ttc][use_tax]=1`, { method: "GET" });
      ...
      if (combinationId && combinationId !== 0) {
        try {
          ...
          }>(`/products/${productId}?price[price_ttc][product_attribute]=${combinationId}&price[price_ttc][use_tax]=1`, { method: "GET" });
          price = priceResponse.product?.price_ttc || price;
        } catch {
          console.warn(`Could not fetch combination price for product ${productId}`);
        }
      }
      ...
      if (combinationId && combinationId !== 0) {
        try {
          ...
          }>(`/combinations/${combinationId}?display=[id,reference]`, { method: "GET" });
...
export async function addProductToCart(
  ...
  const cartData: Record<string, string> = {
    ...
    ...(combinationId && { id_product_attribute: combinationId.toString() }),
  };

  const xmlData = converter.convertRowToXML(cartData);

  try {
    await fetchFromPrestashopApi(`/carts/${cartId}/products`, {
      method: "POST",
